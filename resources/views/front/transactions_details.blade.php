      ...
      <div class="flex flex-col w-full rounded-2xl border border-[#E9E8ED] p-4 gap-4 bg-white">
        <div id="Location" class="flex flex-col gap-2">
          ...
          <div class="flex items-center w-full gap-[10px] bg-white">
            <div class="w-[80px] h-[60px] flex shrink-0 rounded-xl overflow-hidden">
              <img src="{{Storage::url($details->carStore->thumbnail)}}" class="w-full h-full object-cover" alt="thumbnail">
            </div>
            <div class="flex flex-col">
              <div class="flex items-center gap-1">
                <p class="font-semibold w-fit">{{$details->carStore->name}}</p>
                <div class="w-[18px] h-[18px] flex shrink-0">
                  <img src="{{asset('assets/images/icons/verify.svg')}}" alt="verified">
                </div>
              </div>
              <div class="flex items-center gap-[2px]">
                <p class="text-sm leading-[21px] text-[#909DBF]">{{$details->carStore->address}}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div id="Service" class="flex flex-col gap-2">
          ...
          <div class="flex items-center w-full gap-[10px] bg-white justify-between">
            <div class="flex items-center gap-[10px]">
              <div class="w-[60px] h-[60px] flex shrink-0">
                <img src="{{Storage::url($details->carService->icon)}}" alt="icon">
              </div>
              <div class="flex flex-col h-fit">
                <p class="font-semibold">{{$details->carService->name}}</p>
        ...
        <div id="Price-details" class="flex flex-col gap-[10px]">
          <div class="flex items-center justify-between">
            <p class="text-sm leading-[21px]">{{$details->carService->name}} Price</p>
            <p class="font-semibold">Rp {{number_format($details->carService->price,0,',','.')}}</p>
          </div>
          <div class="flex items-center justify-between">
